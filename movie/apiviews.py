...
class MovieListSearch(generics.ListAPIView):
    permission_classes = (IsAuthenticated,)
    queryset = MoviesList.objects.all()
    serializer_class = MoviesListSerializer
    filter_backends = (filters.SearchFilter,)
    #filter_backends = (filters.DjangoFilterBackend,)
    #filterset_fields = ('title', 'url', 'imdbrating', 'year')
    search_fields = ('title', 'url', 'imdbrating', 'year')


class MovieListView(APIView):
    permission_classes = (IsAuthenticated,)
    def get(self, request):
        mlist = MoviesList.objects.all()[:100]
